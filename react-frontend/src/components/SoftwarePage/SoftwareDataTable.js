...
const SoftwareDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    const dt = useRef(null);
    const urlParams = useParams();
    const [showUpload, setShowUpload] =useState(false)

const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.name}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.justifikasi}</p>
...
<Column field="justifikasi" header="Justifikasi" body={pTemplate1} sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Software Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>
        </>
    );
...
export default SoftwareDataTable;