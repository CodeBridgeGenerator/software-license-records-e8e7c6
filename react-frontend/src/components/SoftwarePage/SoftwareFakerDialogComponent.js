...
import softwareFakerFactory from "./softwareFakerFactory";
...
const SoftwareFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = softwareFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("software").create(elem));
...
export default connect(mapState, mapDispatch)(SoftwareFakerDialogComponent);
