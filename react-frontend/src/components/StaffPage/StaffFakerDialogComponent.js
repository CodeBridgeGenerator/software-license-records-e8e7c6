...
import staffFakerFactory from "./staffFakerFactory";
...
const StaffFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = staffFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("staff").create(elem));
...
export default connect(mapState, mapDispatch)(StaffFakerDialogComponent);
